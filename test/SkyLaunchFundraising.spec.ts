...
import StakingRewards from '../build/SkyLaunchFundRaisingV2.json'
...
describe('SkyLaunchFundraisingV2', () => {
    ...
    beforeEach(async () => {
        ...
        const kyc = await fundRaising.KYC_SETTER()
        await fundRaising.grantRole(kyc, wallet.address);
    ...
    it('Only owner can set the FundingTimestamp', async () => {        
        await fundRaising.connect(wallet).add(rewardsToken.address, fundRaisingToken.address, 0, 1000, 2000, expandTo18Decimals(100000), utils.parseEther("0.01"), expandTo18Decimals(500), expandTo18Decimals(10000))
        await expect(fundRaising.connect(nonOwner).setFundingTimestamp(0, 0, 1000, 2000))
            .to.revertedWith("Unauthorized");
        ...
        await fundRaising.connect(wallet).setFundingTimestamp(0, startTime, subEndTime, fundEndTime)
    })

    it('Only owner can set the VestingTimestamp', async () => {        
        await fundRaising.connect(wallet).add(rewardsToken.address, fundRaisingToken.address, 0, 1000, 2000, expandTo18Decimals(100000), utils.parseEther("0.01"), expandTo18Decimals(500), expandTo18Decimals(10000))
        await expect(fundRaising.connect(nonOwner).setVestingTimestamp(0, 0, 1000, 2000))
            .to.revertedWith("Unauthorized");

        const rewardStartTime: number = Math.floor(Date.now() / 1000) + 1000;
        const rewardCliffEndTime: number = rewardStartTime + 1000;
        const rewardEndTime: number = rewardCliffEndTime + 1000;
        mineBlock(provider, Math.floor(Date.now() / 1000));
        await fundRaising.connect(wallet).setVestingTimestamp(0, rewardStartTime, rewardCliffEndTime, rewardEndTime);
    })
    ...
    it('Only valid user can subscribe to upcoming IDO after start time', async () => {
        ...
        await expect(fundRaising.connect(nonOwner).subscribe(0,0, [] ))
            .to.revertedWith("subscription not started");
    ...
    it('Only subscribed users can fund IDO up to their allocation', async () => {
        ...
        await expect(fundRaising.connect(nonOwner).fundSubscription(0, 100))
            .to.revertedWith('not subscribed');
        ...
        await expect(fundRaising.connect(secondStaker).fundSubscription(0, allocation.add(10)))
            .to.revertedWith('Too many tokens provided');
    ...
    it('Vesting rewards can be setup only by owner and claimed by the users', async () => {
        ...
        await expect(fundRaising.connect(wallet).setupVestingRewards(0, rewardAmount, rewardStartTime, rewardCliffEndTime, rewardEndTime)).to.be.revertedWith("Users are still funding");

        mineBlock(provider, fundEndTime + 1);

        await expect(fundRaising.connect(wallet).setupVestingRewards(0, 1000, rewardStartTime, rewardCliffEndTime, rewardEndTime)).to.be.revertedWith("wrong reward amount provided");
        await expect(fundRaising.connect(nonOwner).setupVestingRewards(0, rewardAmount, rewardStartTime, rewardCliffEndTime, rewardEndTime)).to.be.revertedWith("Unauthorized");

        await fundRaising.connect(wallet).setupVestingRewards(0, rewardAmount, rewardStartTime, rewardCliffEndTime, rewardEndTime);

        await expect(fundRaising.connect(secondStaker).claimReward(0)).to.be.revertedWith("Not past cliff");

        mineBlock(provider, rewardCliffEndTime + 1);
        await expect(fundRaising.connect(nonOwner).claimReward(0)).to.be.revertedWith("Not funded"); 
    ...
    it('Fundraising can be claimed by the owner only', async () => {
        ...
        expect(await fundRaising.claimFundRaising(0, wallet.address)).to.emit(fundRaising, "FundRaisingClaimed").withArgs(0, wallet.address, totalRaised);
    })    
})